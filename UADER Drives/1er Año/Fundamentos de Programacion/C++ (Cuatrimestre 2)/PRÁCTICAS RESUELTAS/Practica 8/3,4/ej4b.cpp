...
void getApodo_Detenciones (string linea, string &apodo, int &detenciones);
...
int main(int argc, char *argv[]){
	...
	if (!fichero.fail()){
		...
		while (getline(fichero,linea)){
			getApodo_Detenciones(linea,apodo,detenciones);
...
void getApodo_Detenciones (string linea, string &apodo, int &detenciones){
	int pos = linea.find("|");
	apodo = linea.substr(0,pos);
	detenciones = stoi(linea.substr(pos+1));
}
